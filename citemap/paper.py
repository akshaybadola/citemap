...
class Entry(QGraphicsTextItem):
    ...
    def __init__(self, mmap, index, text, shape=None, coords=None, group=None, data={},
                 paper_data=None):
        super().__init__(text)
        self.mmap = mmap
        self._paper_data = paper_data or {}
        self.set_properties(index, shape, coords, data, text)
        self.set_variables()
    ...
    def serialize(self):
        ...
        data['side'] = self.side
        data['paper_data'] = self._paper_data
    ...
    def set_properties(self, index, shape, coords, data, text):
        ...
        if "family" in data:
            for direction in ["u", "d", "l", "r"]:
                if direction in data["family"]:
                    self.family[direction] = data["family"][direction].copy()
            if "children" in data["family"]:
                self.add_children(data["family"]["children"])
            if "parents" in data["family"]:
                self.add_parents(data["family"]["parents"])
    ...
    def get_relatives_in_direction(self, direction):
        return self.family[direction]

    def add_relative_in_direction(self, index, direction):
        self.family[direction].append(index)

    def add_parents(self, parents):
        if isinstance(parents, int):
            self.family['parents'].add(parents)
        else:
            self.family['parents'].update(set(parents))

    def add_children(self, children):
        if isinstance(children, int):
            self.family['children'].add(children)
        else:
            self.family['children'].update(set(children))
    ...
    def remove(self):
        self.mmap.removeItem(self.shape_item)
        self.mmap.removeItem(self.icon)
        self.mmap.removeItem(self)
